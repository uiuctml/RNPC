...
class CW(Attack):
    ...
    def forward(self, images, labels, attack_ids, data_loader=None): # NEW
        r"""
        Overridden.
        """
        ...
        for step in range(self.steps):
            ...
            if self.targeted:
                loss_values = [self.f(outputs[attack_id], target_labels[:, attack_id]).sum() for attack_id in attack_ids]
                f_loss = torch.mean(torch.stack(loss_values))
            else:
                loss_values = [self.f(outputs[attack_id], labels[:, attack_id]).sum() for attack_id in attack_ids]
                f_loss = torch.mean(torch.stack(loss_values))
            ...
            # NEW for attack
            condition = torch.zeros_like(labels[:, 0], dtype=torch.bool)
            for attack_id in attack_ids:
                pre = torch.argmax(outputs[attack_id].detach(), 1)
                if self.targeted:
                    # We want to let pre == target_labels in a targeted attack
                    condition |= (pre == target_labels[:, attack_id])
                else:
                    # If the attack is not targeted we simply make these two values unequal
                    condition |= (pre != labels[:, attack_id])
