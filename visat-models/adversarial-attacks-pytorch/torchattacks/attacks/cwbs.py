...
class CWBS(Attack):
    ...
    def forward(self, images, labels, attack_ids, data_loader=None):
        r"""
        Overridden.
        """
        ...
        const = torch.full((batch_size, ), self.init_c, dtype=torch.float).to(self.device)
        upper_bound = torch.full((batch_size, ), 1e10).to(self.device)

        o_best_score = torch.full((batch_size, ), -1, dtype=torch.long).to(self.device)
        o_best_Lx = torch.full((batch_size, ), 1e10).to(self.device)

        for _ in range(self.binary_search_steps):
            best_score = torch.full((batch_size, ), -1, dtype=torch.long).to(self.device)
            ...
            for step in range(self.steps):
                ...
                # Calculate loss
                current_Lx = MSELoss(Flatten(adv_images), Flatten(images)).sum(dim=1)

                Lx_loss = current_Lx.sum()

                outputs = self.get_logits(adv_images)[0]
                ...
                if self.targeted:
                    loss_values = [self.f(outputs[attack_id], target_labels[:, attack_id]) for attack_id in attack_ids]
                    f_loss = torch.mean(torch.stack(loss_values))
                else:
                    loss_values = [self.f(outputs[attack_id], labels[:, attack_id]) for attack_id in attack_ids]
                    f_loss = torch.mean(torch.stack(loss_values))
                ...
                pre = torch.argmax(outputs[attack_ids[0]].detach(), 1)
                condition_1 = self.compare(pre, labels[:, attack_ids[0]]) # New
            ...
            pre = torch.argmax(outputs[attack_ids[0]], 1)
